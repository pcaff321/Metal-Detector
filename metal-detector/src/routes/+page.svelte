...
<script lang="ts">
  ...
  import { onMount } from 'svelte';
    ...
    const isExploded = () => keyGiven && _exploded;
    const updateExploded = (exploded: boolean) => _exploded = exploded;
  ...
  $effect(() => {
    if (extremeBeepDuration >= EXTREME_BEEP_DURATION_THRESHOLD && !isExploded()){
        updateExploded(true);
        explosionAudio?.play()
...
{#if keyGiven}
    ...
    <div class="container {withinImage ? 'shakeContainerExtreme' : ''}" on:mousemove|preventDefault={handleMousemove}>
        ...
        <img class="image" style="{isExploded() ? 'display: none' : ''}" style:top={`${divTop}px`} style:left={`${divLeft}px`} {src} alt="Cutie Pie" />
        {/await}
        {/if}
